...
formatTypes[GL_R8I] = GL_BYTE
formatTypes[GL_R8UI] = GL_UNSIGNED_BYTE
formatTypes[GL_R16I] = GL_SHORT
formatTypes[GL_R16UI] = GL_UNSIGNED_SHORT
formatTypes[GL_R32I] = GL_INT
formatTypes[GL_R32UI] = GL_UNSIGNED_INT
...
module.exports = function createTextureSet (
  ...
  function parseFlags (flags, options) {
    ...
    if ('shape' in options) {
      ...
      if (d !== undefined) {
        check(d >= 0 && d <= limits.maxTextureSize, 'invalid depth')
      }
    } else {
    ...
    if ('format' in options) {
      ...
      check(extensions.ext_texture_norm16 ||
        !(formatStr === 'r16' || formatStr === 'r16_norm'),
      'you must enable the EXT_texture_norm16 extension in order to use 16-bit normalized textures.')
  ...
  function parseTexInfo (info, options) {
    ...
    if ('wrap' in options) {
      ...
      if (typeof wrap === 'string') {
        ...
        if (wrap[2] !== undefined) {
          check.parameter(wrap[2], wrapModes)
      ...
      if ('wrapR' in options) {
        ...
        check.parameter(optWrapR, wrapModes)
  ...
  function createTexture3D (a) {
    ...
    function subimage (image, x_, y_, z_, level_) {
      ...
      check(
        ...
        x + imageData.width <= texture.width &&
        y + imageData.height <= texture.height &&
        z + imageData.depth <= texture.depth,
        'texture.subimage write out of bounds')
      check(
        texture.mipmask & (1 << level),
        'missing mipmap data')
